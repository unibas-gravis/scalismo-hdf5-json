...
object DatasetCreationProperties {
  given rw: ReadWriter[DatasetCreationProperties] = upickle.default.readwriter[ujson.Value].bimap[DatasetCreationProperties](
    x => ujson.Obj(),
    x => DatasetCreationProperties())
}

case class HDFDataset(shape : HDFDataSpace,
