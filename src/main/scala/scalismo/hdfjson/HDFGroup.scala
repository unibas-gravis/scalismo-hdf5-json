...
object GroupCreationProperties {
  given  rw : ReadWriter[GroupCreationProperties] = upickle.default.readwriter[ujson.Value].bimap[GroupCreationProperties](
    x => ujson.Obj(),
    x => GroupCreationProperties())
}

case class HDFGroup(attributes : Seq[HDFAttribute] = Seq.empty,
