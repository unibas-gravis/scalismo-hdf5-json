...
object HDFIOTestCases {
  def simpleHDFFile : HDFFile = {
    ...
    val group1 = HDFGroup(Seq.empty, Seq(HDFLink("g11", Collection.groups, group11id, LinkType.H5L_TYPE_HARD)))

    val dataset1id = HDFIdentifier.randomUUID()
    val dataset = HDFDataset(
      HDFDataSpace.simple_dataspace(Array(3)),
      HDFDatatype.floating_point_datatype(FloatingPointDatatype.Base.H5T_IEEE_F32LE),
    ...
    val rootGroup = HDFGroup(
      Seq(
        HDFAttribute("attr1", HDFDatatype.string_datatype(CharSet.H5T_CSET_UTF8, StrPad.H5T_STR_NULLTERM, Length.H5T_VARIABLE), HDFDataSpace.simple_dataspace(Array(3)), ujson.Str("abc"))
      ),
      Seq(HDFLink("g1", Collection.groups, group1id, LinkType.H5L_TYPE_HARD), HDFLink("d1", Collection.datasets, dataset1id, LinkType.H5L_TYPE_HARD)
      )
    )
